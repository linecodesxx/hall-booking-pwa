...
export function Schedule({ go, user }) {
  ...
  const rangeDates = view === 'week' ? weekDays(date) : view === 'month' ? monthDays(date) : [];
  const { data: range = [], isLoading: rangeLoading } = useRangeSchedule(rangeDates);
  ...
  const handleUpdate = async (data) => {
    try {
      ...
      if (res?.booking) setSelectedBooking(res.booking);
    } catch (err) {
      console.error(err);
    }
  };
    ...
    <div className="schedule-layout">
      ...
      {dayBookings && (
        <DayBookingsModal
          ...
          onCreateBooking={() => go(`/new-booking?date=${dayBookings.date}`)}
          onBookingClick={onBookingClick}
