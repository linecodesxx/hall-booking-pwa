...
import { addDays, niceDate, todayISO } from '../utils';
...
export function Today({ go, user }) {
  ...
  const { data: pendingList = [] } = useBookings('pending', {
  ...
  const futureBookings = (range || [])
    .flatMap((d) =>
      (d?.halls || []).flatMap((hall) =>
        (hall?.bookings || [])
          .filter(
            (b) => b.date !== today && b.status === 'approved' && b.temporal_status === 'upcoming',
          )
                ...
                {bookings.map((b) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => go(`/schedule?booking=${b.id}`)}
                    >
                      <span className="hall-dot" style={{ backgroundColor: hColor }} />
                      <div>
                        <strong>
