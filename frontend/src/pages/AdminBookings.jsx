	...
	const actGroup = async (groupId, action) => {
		try {
			...
			if (action === "approve" && result.errors?.length) {
				...
				setNotice(
					action === "approve" ? "Группа подтверждена." : "Группа отклонена.",
				);
			}
		} catch (err) {
			setNotice({ text: err.message, type: "error" });
	...
	return (
		<div className="admin-bookings-layout">
			...
			{grouped.map(([groupId, group]) => {
				...
				return (
					<section key={groupId} className="card" style={{ marginBottom: 12, padding: 0, overflow: "hidden" }}>
						<div
							className="card-head"
							style={{
								cursor: "pointer",
							...
							<div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, width: "100%" }}>
								<div style={{ display: "flex", alignItems: "center", gap: 8, flex: 1, minWidth: 0 }}>
									...
									<span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
										...
										{group[0].user_name && <span style={{ color: "var(--muted)", marginLeft: 8 }}>{group[0].user_name}</span>}
										<span style={{ color: "var(--muted)", marginLeft: 8, fontSize: 13 }}>— {group.length} броней</span>
									</span>
								</div>
								{allPending.length === group.length && (
									<div style={{ display: "flex", gap: 6, alignItems: "center" }} onClick={(e) => e.stopPropagation()}>
										<input
											...
											onChange={(e) =>
												setGroupComments((prev) => ({ ...prev, [groupId]: e.target.value }))
											}
											style={{ maxWidth: 160, fontSize: 13, padding: "4px 8px" }}
										/>
										<button className="primary" style={{ fontSize: 13, padding: "4px 10px" }} onClick={() => actGroup(groupId, "approve")}>
											Подтвердить всё
										</button>
										<button className="secondary danger" style={{ fontSize: 13, padding: "4px 10px" }} onClick={() => setConfirmGroupReject(groupId)}>
											Отклонить всё
										</button>
									</div>
								)}
			...
			{/* Ungrouped bookings */}
			{ungrouped.length > 0 && <BookingList
				bookings={ungrouped}
				adminActions={(booking) =>
					booking.status === "pending" ? (
						<div className="admin-actions">
