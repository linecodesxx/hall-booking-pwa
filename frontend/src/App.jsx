  ...
  return (
    <>
      <div style={{ display: route.startsWith('/new-booking') ? '' : 'none' }}>
        <ErrorBoundary>
          <NewBooking go={go} setNotice={setNotice} user={user} query={query} key={route} />
        </ErrorBoundary>
      </div>
      ...
      <div
        style={{
          display: route === '/admin' || route === '/admin/bookings' ? '' : 'none',
        }}
      >
        <ErrorBoundary>
...
export function App() {
  ...
  const setNotice = useCallback((msg) => {
    const entry = typeof msg === 'string' ? { text: msg, type: 'success' } : msg;
    setNoticeRaw(entry);
        ...
        <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
          {PUSH_ENABLED && 'Notification' in window && Notification.permission === 'default' && (
            <button
              ...
              title="Включить уведомления о новых заявках"
            >
              <Bell size={18} />
            </button>
          )}
