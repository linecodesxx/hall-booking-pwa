      ...
      <EditBookingModal
        ...
        onSave={async () => {
          ...
          for (const k of Object.keys(form)) {
            if (String(form[k]) !== String(booking[k])) changedFields[k] = form[k];
          }
          ...
          <article className="booking-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="modal-title-row">
                <h2>{booking.title}</h2>
                <span className={`badge ${booking.status}`}>{labels[booking.status]}</span>
              </div>
              ...
              {canAct && (
                <div className="two-cols" style={{ marginTop: 8 }}>
                  ...
                  {!isTerminal && (
                    <button className="secondary danger" onClick={() => setConfirmCancel(true)}>
                      Отменить
                    </button>
                  )}
                </div>
    ...
    <div className="modal-overlay" onClick={onCancel}>
      <article className="booking-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <label>
            Название
            <input value={booking.title} onChange={(e) => onFieldChange('title', e.target.value)} />
          </label>
          <label>
            Помещение
            <select
