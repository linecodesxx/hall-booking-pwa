...
function WeekDayRow({ day, bookings, onDayClick, onDayBookings, onBookingClick }) {
  ...
  return (
    <div className="week-day-row">
      <button className="week-day-label" onClick={() => onDayBookings({ date: day, bookings })}>
        <span className="week-day-name">{dayName}</span>
        <span className="week-day-date">{new Date(`${day}T12:00:00`).getDate()}</span>
      </button>
      <div className="week-day-bookings" onClick={() => bookings.length === 0 && onDayClick(day)}>
        {bookings.length ? (
          bookings.map((booking) => (
            ...
            >
              <span className="week-booking-time">{booking.start_time}</span>
              <span className="week-booking-title">{booking.title || 'Занято'}</span>
              <span
                className="hall-dot"
                style={{
          ...
          return (
            ...
            >
              ...
              {bookings.length
                ...
                : ''}
              {bookings.length > 3 && <small>+{bookings.length - 3}</small>}
            </div>
          );
        })}
      </div>
