import { dayEndHour, dayStartHour, hourRows, niceDate, timeToMinutes } from '../utils';
...
export function CalendarGrid({ halls, date, go, onCellClick, onBookingClick }) {
  ...
  return (
    <section className="calendar-card card">
      ...
      <div className="calendar-scroll" aria-label="Календарная сетка бронирований" role="region">
          ...
          {items.map((hall) => (
            <div className="hall-column" key={hall.id}>
              ...
              {(hall.bookings || []).map((booking) => {
                const topHours =
                  Math.max(0, timeToMinutes(booking.start_time) - dayStartHour * 60) / 60;
                const heightHours =
                  (timeToMinutes(booking.end_time) - timeToMinutes(booking.start_time)) / 60;
                return (
                  <button
                  ...
                  >
                    ...
                    <span>{booking.title || 'Занято'}</span>
                  </button>
                );
              })}
            </div>
