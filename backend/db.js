  ...
  prepare(sql) {
    return {
      ...
      run(...params) {
        ...
        const result = { lastInsertRowid: Number(database.exec('SELECT last_insert_rowid() AS id')[0]?.values[0]?.[0] || 0) };
        persist();
        return result;
      }
    };
...
export async function initDb() {
  ...
  if (!count) {
    const insert = db.prepare('INSERT INTO halls (name, description, color) VALUES (?, ?, ?)');
    [
    ...
    const palette = ['#2563eb', '#20a36b', '#f59e0b', '#e11d48', '#7c3aed', '#0f766e', '#ea580c', '#0891b2'];
    db.prepare('SELECT id FROM halls WHERE color IS NULL OR color = \'\' ORDER BY id').all().forEach((hall, index) => {
      db.prepare('UPDATE halls SET color = ? WHERE id = ?').run(palette[index % palette.length], hall.id);
    });
  }
}

export function findApprovedConflict({ hall_id, date, start_time, end_time, excludeId }) {
  return db.prepare(`
  ...
  `).all(hall_id, date, start_time, end_time, excludeId ?? null, excludeId ?? null);
}

export function findPendingOverlap({ hall_id, date, start_time, end_time, excludeId }) {
  return db.prepare(`
    SELECT b.*, h.name AS hall_name, u.name AS user_name
    FROM bookings b
    JOIN halls h ON h.id = b.hall_id
