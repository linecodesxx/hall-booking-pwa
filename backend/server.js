...
const requiredVariables = ['JWT_SECRET', 'USER_INVITE_CODE', 'ADMIN_INVITE_CODE'];
...
try {
  ...
  throw new Error(`Invalid APP_TIME_ZONE: ${APP_TIME_ZONE}`);
}

await initDb();

function appDateTime(date = new Date()) {
...
const LOGIN_RATE_LIMIT = Number(process.env.LOGIN_RATE_LIMIT || 50);
...
if (PUSH_ENABLED) {
  ...
  if (fs.existsSync(vapidPath)) {
    ...
    fs.writeFileSync(vapidPath, JSON.stringify(vapidKeys, null, 2), { mode: 0o600 });
  }
...
if (!corsOrigin) throw new Error('CORS_ORIGIN is required in production');
const corsOptions = {
...
function loginRateLimit(req, res, next) {
  ...
  res.set('RateLimit-Remaining', String(Math.max(0, LOGIN_RATE_LIMIT - entry.count)));
  ...
  asyncHandler(async (req, res) => {
    ...
    if (existing) {
      if (existing.password_hash) {
        ...
        db.prepare('UPDATE users SET role = ?, last_login_at = CURRENT_TIMESTAMP WHERE id = ?').run(
          nextRole,
          existing.id,
        );
      } else {
      ...
      const credentials = hashPassword(password);
      const result = db
        .prepare('INSERT INTO users (name, role, password_hash, password_salt) VALUES (?, ?, ?, ?)')
...
app.get(
  ...
  asyncHandler(async (_req, res) => {
    res.json({
      halls: db.prepare('SELECT * FROM halls WHERE is_active = 1 ORDER BY name').all(),
    });
  }),
);
...
app.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = {
      bookings: rows.map((row) => publicBooking(row, req.user)),
    };
    if (req.user.role === 'admin') {
...
app.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const rows = bookingSelect("WHERE b.date = ? AND b.status IN ('pending', 'approved')", [
      req.query.date,
    ]);
    res.json({
      date: req.query.date,
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (findApprovedConflict(data).length)
      throw Object.assign(new Error('Это время уже занято. Выберите другое время.'), {
        status: 409,
      });
    ...
    if (isAdmin) {
      if (findPendingOverlap(data).length)
        throw Object.assign(
          new Error('На это время есть ожидающие заявки. Выберите другое время.'),
          { status: 409 },
        );
      const result = db
        .prepare(`
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!Array.isArray(items) || items.length === 0) throw badRequest('Нет броней для создания');
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      const hall = db
        ...
        .get(data.hall_id);
      if (!hall) throw badRequest(`Бронь #${i + 1}: помещение недоступно`);
      if (findApprovedConflict(data).length)
        throw Object.assign(
          new Error(`${data.date} ${data.start_time}-${data.end_time}: время уже занято`),
          { status: 409 },
        );
      validated.push(data);
    }
...
app.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const pub = publicBooking(bookingSelect('WHERE b.id = ?', [req.params.id])[0], req.user);
...
app.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (conflicts.length)
      throw Object.assign(
        new Error('Нельзя подтвердить: время пересекается с подтверждённой бронью'),
...
app.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!group.length) throw badRequest('В группе нет заявок, ожидающих подтверждения');
    ...
    for (const booking of group) {
      ...
      const pub = publicBooking(bookingSelect('WHERE b.id = ?', [booking.id])[0], req.user);
      wss.broadcast({ type: 'booking.updated', booking: pub });
      sendPush(booking.user_id, {
        title: 'Заявка подтверждена',
        body: `${pub.title} • ${pub.date} ${pub.start_time}-${pub.end_time}`,
...
app.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!existing) throw Object.assign(new Error('Заявка не найдена'), { status: 404 });
    const updates = {};
    ...
    if (req.body.start_time !== undefined || req.body.end_time !== undefined) {
      ...
      if (!start_time || !end_time) throw badRequest('Укажите время');
      if (start_time >= end_time)
        throw badRequest('Время начала должно быть раньше времени окончания');
      updates.start_time = start_time;
      updates.end_time = end_time;
    }
    if (Object.keys(updates).length === 0) throw badRequest('Нет полей для обновления');
    const sets = Object.keys(updates)
      .map((k) => `${k} = ?`)
      .join(', ');
    db.prepare(`UPDATE bookings SET ${sets}, updated_at = CURRENT_TIMESTAMP WHERE id = ?`).run(
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!endpoint || !keys?.p256dh || !keys?.auth) throw badRequest('Invalid subscription');
    try {
      db.prepare(
        'INSERT INTO push_subscriptions (user_id, endpoint, p256dh, auth) VALUES (?, ?, ?, ?)',
      ).run(req.user.id, endpoint, keys.p256dh, keys.auth);
    } catch (_e) {
      db.prepare('UPDATE push_subscriptions SET p256dh = ?, auth = ? WHERE endpoint = ?').run(
        keys.p256dh,
        keys.auth,
        endpoint,
      );
...
app.get('/api/health', (_req, res) => {
  res.json({ ok: true, timeZone: APP_TIME_ZONE });
});
...
server.listen(PORT, process.env.HOST || '0.0.0.0', () => {
  console.log(`Hall booking API running on port ${PORT}`);
});
